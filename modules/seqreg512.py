...
class Reg(nn.Module):
    def __init__(self, size=train_image_size, mode='bilinear'):
        ...
        self.decoder = nn.Sequential(nn.ConvTranspose3d(3, 3, (2, 2, 2), (2, 2, 2)),
    ...
    def one_forward(self, x, moving_image=None):  # x: 网络输入  moving_image: 待配准图像（组） 对单个时间点进行配准，moving_image是该时间点图像（或该时间点图像加掩码）
        if x.shape[-3:] != torch.Size(train_image_size):  # 如果大小和网络接受大小不一致，需要进行重采样
            ...
            # down_image = self.encoder(x).type(torch.float32)
            moved_grid, flow = self.get_field(down_image)
            moved_grid = self.decoder(moved_grid.permute(0, 4, 1, 2, 3)).permute(0, 2, 3, 4, 1)
        else:
...
def main():
    ...
    a = np.load(r'D:\jcr\reg\data\ours\cheguangxun.npy')
    a = np.transpose(a, [0, 3, 1, 2])
    a = np.reshape(a, [1, 2, -1, 256, 512, 512])
    x = a[:, 0]
    mask = a[:, 1]
    ...
    with torch.no_grad():
        ...
        plt.subplot(121)
        plt.imshow(x[0, 0, 128, :, :].cpu(), cmap='gray')
        plt.subplot(122)
        plt.imshow(y[0, 0, 128, :, :].cpu(), cmap='gray')
        plt.show()

        plt.subplot(121)
        plt.imshow(mask[0, 0, 128, :, :].cpu(), cmap='gray')
        plt.subplot(122)
        plt.imshow(rm[0, 0, 128, :, :].cpu(), cmap='gray')
        plt.show()
