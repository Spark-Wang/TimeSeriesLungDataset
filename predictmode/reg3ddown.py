...
class Predicter():
    ...
    def predict_once(self, number, net, image, ground_truth, name):
        ...
        if down:
            ...
            down_ground_truth = F.interpolate(ground_truth.float(), scale_factor=0.25, mode='trilinear')
            down_pred_image, down_pred_mask, field = net(down_image, down_ground_truth.type(torch.float32), False, True)

            
            moving_image = image[:, 1:]
            # fixed_mask = mask[:, :1]
            moving_mask = ground_truth[:, 1:]
            field = F.interpolate(field.permute(0, 4, 1, 2, 3), scale_factor=4).permute(0, 2, 3, 4, 1)
            
            pred_image = net.use_field(moving_image, field)
            pred_mask = net.use_field(moving_mask.type(torch.float32), field)
        else:
        ...
        pred_image, pred_mask = pred_image[0, 0], pred_mask[0, 0].type(torch.uint8)
        ...
        if cncc:
            if down:
                ncc = pncc(down_pred_image[0, 0], down_image[0, 0]).item()
            else:
                ncc = pncc(pred_image, fixed_image).item()
