...
train_path = r'D:\jcr\reg\data\downours'
val_path = 0.8
...
def main():
    ...
    for name, image, time_diff, mask in loader:
        ...
        for i in range(image.shape[1]):
            plt.subplot(221)
            plt.imshow(image[0, i, 25, :, :], cmap='gray')
            plt.subplot(222)
            plt.imshow(mask[0, i, 25, :, :], cmap='gray')
            plt.subplot(223)
            plt.imshow(image[0, i, 26, :, :], cmap='gray')
            plt.subplot(224)
            plt.imshow(mask[0, i, 26, :, :], cmap='gray')
            plt.show()
        break
